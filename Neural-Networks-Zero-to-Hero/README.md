# Neural Networks: Zero to Hero 学习笔记

从零开始理解神经网络，并最终亲手构建一个 LLM。

本目录记录 [Andrej Karpathy 的 Neural Networks: Zero to Hero](https://karpathy.ai/zero-to-hero.html) 课程学习过程。笔记不追求逐字复述视频，而是围绕「核心问题 → 原理 → 实现 → 易错点 → 复习与练习」整理，方便回顾和继续扩展。
